...
The Pegasys Factory entity is responsible for storing aggregate information across all Pegasys pairs. It can be used to view stats about total liquidity, volume, amount of pairs and more. There is only one PegasysFactory entity in the subgraph.

| Field Name        | Value Type | Description                                                     |
| ----------------- | ---------- | --------------------------------------------------------------- |
| id                | ID         | factory address                                                 |
| pairCount         | Int        | amount of pairs created by the Pegasys factory                  |
...
| factory              | PegasysFactory      | reference to Pegasys factory entity                                                                                 |
...
A user entity is created for any address that provides liquidity to a pool on Pegasys. This entity
...
Transaction entities are created for each Ethereum transaction that contains an interaction within Pegasys contracts. This subgraph tracks Mint, Burn, and Swap events on the Pegasys core contracts. Each transaction contains 3 arrays, and at least one of these arrays has a length of 1.
...
Mint entities are created for every emitted Mint event on the Pegasys core contracts. The Mint entity stores key data about the event like token amounts, who sent the transaction, who received the liquidity, and more. This entity can be used to track liquidity provisions on pairs.
...
Burn entities are created for every emitted Burn event on the Pegasys core contracts. The Burn entity stores key data about the event like token amounts, who burned LP tokens, who received tokens, and more. This entity can be used to track liquidity removals on pairs.
...
| mostLiquidTokens  | [TokenDayData!] | tokens with most liquidity in Pegasys                                            |
