...
```solidity
require(DAI.approve(address(PegasysV1Router02), amountIn), 'approve failed.');
```
...
path[0] = address(DAI);
path[1] = PegasysV1Router02.WETH();
PegasysV1Router02.swapExactTokensForETH(amountIn, amountOutMin, path, msg.sender, block.timestamp);
```
...
However, when an off-chain price can't be used, an on-chain oracle should be used instead. Determining the best oracle for a given situation is a not part of this guide, but for more details on the Pegasys V1 approach to oracles, see [Oracles](../../concepts/core-concepts/oracles).
